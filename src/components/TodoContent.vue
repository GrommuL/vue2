<template>
	<div class="content">
		...
		<h3 class="content__empty"></h3>
		<p class="content__count">
			Количество задач:
			<span class="content__count-num">{{
				this.$store.getters.getAllTodos.length
			}}</span>
		</p>
	</div>
</template>
...
.content__top-btn {
	...
	border: 1px solid rgb(52, 94, 178);
	border-radius: 5px;
	cursor: pointer;
	color: rgb(52, 94, 178);
	padding: 5px;
}
.content__top-btn.active {
	background: rgb(52, 94, 178);
	color: white;
}
